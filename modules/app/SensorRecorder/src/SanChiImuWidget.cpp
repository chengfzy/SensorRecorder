...
// Constructor
SanChiImuWidget::SanChiImuWidget(QWidget* parent)
    : ISensorWidget(parent),
      saveFile_(
          QDir(QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + "/imuSanChi.csv").absolutePath()) {
    setupUI();
}

// Set the device list
void SanChiImuWidget::setDevices(const QStringList& devices) {
    deviceComboBox_->clear();
    deviceComboBox_->addItems(devices);
    deviceComboBox_->setCurrentIndex(0);
}

// Set current selected IMU
void SanChiImuWidget::setCurrentDevice(const size_t& index) {
...
// Set save file
void SanChiImuWidget::setSaveFile(const QString& file) { saveFileLineEdit_->setText(file); }

// Initialize
void SanChiImuWidget::init() {
    // create recorder
    if (recorder_) {
        recorder_->setDevice(deviceComboBox_->currentText().toStdString());
    } else {
        recorder_ = make_shared<SanChiImuRecorder>(deviceComboBox_->currentText().toStdString());
    }
    ...
    recorder_->addCallback(SanChiImuRecorder::CallBackStarted, [&] {
    ...
    recorder_->addCallback(SanChiImuRecorder::CallBackFinished, [&] { fileStream_.close(); });
    ...
    // set
    recorder_->setFrequency(freqSpinBox_->value());
...
void SanChiImuWidget::capture() { captureMode_ = SensorCaptureMode::Once; }

// Record IMU readings
void SanChiImuWidget::record() {
    if (isRecord_) {
...
// Setup UI
void SanChiImuWidget::setupUI() {
    // device
    deviceComboBox_ = new QComboBox;
    deviceComboBox_->addItem("/dev/ttyUSB0");  // add default device name
    deviceComboBox_->setEditable(true);
    deviceComboBox_->setCurrentIndex(0);
    // frequency
    freqSpinBox_ = new QSpinBox;
    freqSpinBox_->setRange(10, 100);
    freqSpinBox_->setSingleStep(10);
    freqSpinBox_->setValue(100);
    ...
    settingLayout->addWidget(new QLabel("Frequency(Hz)"), 1, 0);
    settingLayout->addWidget(freqSpinBox_, 1, 1, 1, 2);
    settingLayout->addWidget(new QLabel("Timestamp Retrieve Method"), 2, 0);
    settingLayout->addWidget(timestampMethodComboBox_, 2, 1, 1, 2);
    settingLayout->addWidget(new QLabel("Save File"), 3, 0);
    settingLayout->addWidget(saveFileLineEdit_, 3, 1);
    settingLayout->addWidget(selectFileButton_, 3, 2);

    // init IMU button
    initButton_ = new QPushButton(QIcon(":/Icon/Init"), "");
    initButton_->setStatusTip("Initialize");
    connect(initButton_, &QPushButton::clicked, this, &SanChiImuWidget::init);
    // live show
    liveButton_ = new QPushButton(QIcon(":/Icon/Live"), "");
    liveButton_->setStatusTip("Start live show");
    liveButton_->setEnabled(false);
    connect(liveButton_, &QPushButton::clicked, this, &SanChiImuWidget::live);
    // capture
    captureButton_ = new QPushButton(QIcon(":/Icon/Capture"), "");
    captureButton_->setStatusTip("Capture one reading");
    captureButton_->setEnabled(false);
    connect(captureButton_, &QPushButton::clicked, this, &SanChiImuWidget::capture);
    // record
    recordButton_ = new QPushButton(QIcon(":/Icon/Record"), "");
    recordButton_->setStatusTip("Record all readings");
    recordButton_->setEnabled(false);
    connect(recordButton_, &QPushButton::clicked, this, &SanChiImuWidget::record);
...
void SanChiImuWidget::enableSettingWidget(bool enable) {
    deviceComboBox_->setEnabled(enable);
    freqSpinBox_->setEnabled(enable);
